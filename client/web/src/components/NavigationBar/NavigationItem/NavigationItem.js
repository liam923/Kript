...
import {DatumType} from '../../Datum/DatumType/DatumType';

const navigationItem = (props) => {
  let output = null;

  if (props.navType === 'all') {
    output = <span onClick={props.clicked} className={props.active
        ? styles.active
        : null}><AllInclusiveIcon className={styles.icon}/><span
        className={styles.NavText}>All</span></span>;
  } else {
    output = DatumType.getType(props.navType).navItem(props);
  }
